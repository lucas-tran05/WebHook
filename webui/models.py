...
class FieldSchema(BaseModel):
    ...
    type: str = Field(
        ..., description="Data type: string, integer, float, boolean, email, url, json, array"
    )
...
class ScanRequest(BaseModel):
    ...
    sample_valid_payload: str = Field(
        default='{"event": "test", "data": "sample"}', description="Sample payload (fallback)"
    )
    payload_schema: Optional[List[FieldSchema]] = Field(
        default=None, description="Field-based schema for automatic test generation"
    )
